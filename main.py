...
async def list_expenses(start_date,end_date):
    '''List all expenese entries from the database'''
    try:
...
@mcp.tool()
async def delete_expenses(id = None):
    '''removes all the rocords from the the expenses table'''
    try:
        async with aiosqlite.connect(DB_PATH) as c:
            query = ( '''
...
@mcp.tool()
async def drop_expenses():
    '''removes expenses table'''
    try:
        async with aiosqlite.connect(DB_PATH) as c:
            query = ( '''
                    Drop TABLE expenses
        ...
        return {'status':'error',"message":f"Error in drop expenses:{str(e)}"}

    
@mcp.tool()
async def summarize(start_date,end_date,category = None):
    '''Summarize expenses by category within an inclusicve date range.'''
    try:
        async with aiosqlite.connect(DB_PATH) as c:
            query = (
                '''
        ...
        return{"status":'Error',"message":f"Error while summairzing {str(e)}"}
